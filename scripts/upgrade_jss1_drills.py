...
def process_jss1_file(filepath, week_styles):
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()

    # Change static const to static final
    content = content.replace('static const List<JSS1Lesson> weeks = [', 'static final List<JSS1Lesson> weeks = [')

    parts = content.split('JSS1Lesson(')
    header = parts[0]
    new_parts = [header]
    ...
    for part in parts[1:]:
        ...
        vocab_matches = re.findall(
            r'JSS1VocabItem\s*\(\s*french:\s*[\'"]((?:[^\'"]|(?<=\\)\'|(?<=\\)")*)[\'"]\s*,\s*phonetics:\s*[\'"][^\'"]*[\'"]\s*,\s*english:\s*[\'"]((?:[^\'"]|(?<=\\)\'|(?<=\\)")*)[\'"]',
            part
        )

        drill_count = len(re.findall(r'JSS1InteractiveExercise\s*\(', part))
        ...
        new_drills.append(f"""        JSS1InteractiveExercise(
...
{pairs_code}
        ...
        new_drills.append(f"""        JSS1InteractiveExercise(
          prompt: 'Which of the following is an essential habit for mastering French in JSS1?',
    ...
    new_content = 'JSS1Lesson('.join(new_parts)
...
if __name__ == '__main__':
    term1_styles = {
        1: 'interactiveStudio',
        2: 'conversationalDialogue',
        3: 'conversationalDialogue',
        4: 'phoneticSoundboard',
    ...
    base = r"c:\Users\DELL\Desktop\Lang Huey\JSS1_FRENCH\lib\data"
    process_jss1_file(os.path.join(base, "jss1_term1_lessons.dart"), term1_styles)
    process_jss1_file(os.path.join(base, "jss1_term2_lessons.dart"), term2_styles)
    process_jss1_file(os.path.join(base, "jss1_term3_lessons.dart"), term3_styles)
