...
from core import search, DATA_DIR, CSV_CONFIG, BM25SearchProvider, IDomainSearchProvider
...
class IOutputFormatter(ABC):
    """Interface for design system output formatters."""

    @abstractmethod
    def format(self, design_system: Dict[str, Any]) -> str:
        pass
...
class IDataLoader(ABC):
    ...
    @abstractmethod
    def load_reasoning_data(self) -> List[Dict]:
        pass


class IPersistenceManager(ABC):
    """Interface for design system persistence."""
...
class IReasoningEngine(ABC):
    """Interface for reasoning rule application."""
...
class ISearchCoordinator(ABC):
    """Interface for coordinating multi-domain searches."""

    @abstractmethod
    def multi_domain_search(
        self, query: str, style_priority: List[str] = None
    ) -> Dict[str, Any]:
        pass


class IRecommendationBuilder(ABC):
    """Interface for building design system recommendations."""

    @abstractmethod
    def build_recommendation(
        self, query: str, search_results: Dict[str, Any], reasoning: Dict[str, Any]
    ) -> Dict[str, Any]:
        pass
...
class CSVDataLoader(IDataLoader):
    """Loads reasoning data from CSV files."""

    def __init__(self, data_dir: Path, reasoning_file: str):
        self.data_dir = data_dir
        self.reasoning_file = reasoning_file

    def load_reasoning_data(self) -> List[Dict]:
        """Load reasoning rules from CSV."""
        filepath = self.data_dir / self.reasoning_file
...
class ReasoningEngine(IReasoningEngine):
    """Handles reasoning rules and their application."""

    def __init__(self, data_loader: IDataLoader = None):
        self.data_loader = data_loader or CSVDataLoader(DATA_DIR, REASONING_FILE)
        self.reasoning_data = self.data_loader.load_reasoning_data()
...
# ============ SEARCH COORDINATOR ============
class SearchCoordinator(ISearchCoordinator):
    """Coordinates multi-domain searches using injected search providers."""

    def __init__(self, search_provider: IDomainSearchProvider):
    ...
    def multi_domain_search(
        ...
        for domain, config in SEARCH_CONFIG.items():
            if domain == "style" and style_priority:
                ...
                results[domain] = self.search_provider.search(
                    combined_query, domain, config["max_results"]
                )
            else:
                results[domain] = self.search_provider.search(
                    query, domain, config["max_results"]
                )
        return results
...
class RecommendationBuilder(IRecommendationBuilder):
    """Builds design system recommendations from search results."""

    def __init__(self, search_coordinator: ISearchCoordinator):
        self.search_coordinator = search_coordinator

    def _select_best_match(
        self, results: List[Dict], priority_keywords: List[str]
    ) -> Dict:
    ...
    def _extract_results(self, search_result: Dict) -> List[Dict]:
    ...
    def build_recommendation(
        ...
        """Build the final design system recommendation."""
        # Select best matches from each domain using priority
        ...
        return {
            "category": search_results.get("product", {})
            .get("results", [{}])[0]
            .get("Product Type", "General"),
            "pattern": {
...
# ============ DESIGN SYSTEM GENERATOR (REFACTORED) ============
class DesignSystemGenerator:
    """Generates design system recommendations using dependency injection."""

    def __init__(
        self,
        reasoning_engine: IReasoningEngine = None,
        search_provider: IDomainSearchProvider = None,
        recommendation_builder: IRecommendationBuilder = None,
    ):
        self.reasoning_engine = reasoning_engine or ReasoningEngine()
        self.search_coordinator = SearchCoordinator(
            search_provider or BM25SearchProvider(CSV_CONFIG, DATA_DIR)
        )
        self.recommendation_builder = recommendation_builder or RecommendationBuilder(
            self.search_coordinator
        )

    def generate(self, query: str, project_name: str = None) -> Dict[str, Any]:
        """Generate complete design system recommendation."""
        # Step 1: First search product to get category
        product_result = self.search_coordinator.search_provider.search(
            query, "product", 1
        )
        ...
        reasoning = self.reasoning_engine.apply_reasoning(category)
        style_priority = reasoning.get("style_priority", [])

        # Step 3: Multi-domain search with style priority hints
        search_results = self.search_coordinator.multi_domain_search(
            query, style_priority
        )
        search_results["product"] = product_result  # Reuse product search

        # Step 4: Build final recommendation
        recommendation = self.recommendation_builder.build_recommendation(
            query, search_results, reasoning
        )

        # Step 5: Add project name
        recommendation["project_name"] = project_name or query.upper()

        return recommendation


# ============ OUTPUT FORMATTER CLASSES ============
class ASCIIOutputFormatter(IOutputFormatter):
    """Formats design system as ASCII box."""

    def __init__(self, box_width: int = 90):
...
class MarkdownOutputFormatter(IOutputFormatter):
    """Formats design system as markdown."""
...
def persist_design_system(
    ...
    """Helper function to persist design system."""
    pm = PersistenceManager()
    return pm.persist_design_system(design_system, page, output_dir, page_query)
...
def generate_design_system(
    ...
    page: str = None,
    output_dir: str = None,
    formatter: IOutputFormatter = None,
) -> str:
    """
    ...
    Args:
        ...
        formatter: Optional output formatter (overrides output_format)
    ...
    if persist:
        ...
        persistence_manager.persist_design_system(
            design_system, page, output_dir, query
        )

    # Use provided formatter or create based on output_format
    if formatter:
        return formatter.format(design_system)

    if output_format == "markdown":
        formatter = MarkdownOutputFormatter()
    else:
        formatter = ASCIIOutputFormatter()
...
class PersistenceManager(IPersistenceManager):
    """Manages persistence of design systems to files."""
